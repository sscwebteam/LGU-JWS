...
*/            echo json_encode($arrDates);
    ...
    switch (strtolower($_POST['report'])){
        case 'jws1': //FFU#40
            ...
            $Parameters=trim($_POST['year_value'])."-".trim($_POST['month_value']);
        ...
        case 'other_payment': //FFU#40
            ...
            if($other_payments=='app-fee-par'){//user initiate partial payment FFU#49
                    ...
                    for($i=1;$i<3;$i++){
                        ...
                        $sql="update {$LedgerTable} set AF='{$amount}',OR_num='{$or_num}',OR_date='{$or_date}',total='{$amount}' where reading_date='{$InitialReadingDate}'"; //insert into
                        $e=new Exception();
               ...
               //notes:
                    ...
                    */
                    ...
                    $row=mysql_fetch_array($qry);
                    $InitialReadingDate=$row['reading_date'];
                    $sql="update {$LedgerTable} set AF='{$Payment}',OR_num='{$or_num}',OR_date='{$or_date}',total='{$Payment}' where reading_date='{$InitialReadingDate}'";
                    $e=new Exception();
