...
class cls_user_set extends cls_user_get { //user setters
    ...
    /**
    * Method: insert the OR Number as payment for new registration
    * @param [string] account number of the concessionaire
    * @param [string] OR number of the concessionaire
    * @param [string] dates internally generated
    */
    public function NewRegCons_OrNo($AccountNo,$OR_No){
        $date=date('Y-m-s');
        include_once 'db_conn.php';
        $sql="update profile set or_no='{$OR_No}',or_date='{$date}' where acct_no='{$AccountNo}'";
        $e=new Exception();
