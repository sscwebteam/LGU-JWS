            ...
            case 'download_ledger':
                ...
                $data = array();
                    while($row = mysql_fetch_assoc($sql)){
                        ...
                        'loan_mlp'=>$row['loans_MLP'],
                        'loan_mf'=>$row['loans_MF'],
                        'total'=>$row['total'],
                        'or_no'=>$row['OR_num'],
                        'or_date'=>$row['OR_date'],
                        'remarks'=>$row['remarks']));
                 ...
                 #get the profile from another table before appending it to the results above
                    ...
                    array_push($data,array('acct_no' => $profile['acct_no'],'applicant'=>$profile['applicant'],'address'=>$profile['address_brgy'],'date_installed'=>$profile['date_installed'],'serial_no'=>$profile['serial_no'],'brand'=>$profile['brand']));
                 #send it to the function for excel download
                    listings_type::download_ledger($data);
                break;    
}
